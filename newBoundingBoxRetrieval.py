...
def getBoundingBoxData(city):
	urlbase = """http://maps.googleapis.com/maps/api/geocode/json?sensor=true&address="""
	...
	package = entirePayload['results']
	cities = []
	for city in package:
		if not city['geometry'].has_key('bounds'):
			continue
		cityDetails = {}
		cityDetails['cityName'] = city['formatted_address'].encode('ascii', 'ignore')
		cityDetails['ne_lat'] = city['geometry']['bounds']['northeast']['lat']
		cityDetails['ne_lng'] = city['geometry']['bounds']['northeast']['lng']
		cityDetails['sw_lat'] = city['geometry']['bounds']['southwest']['lat']
		cityDetails['sw_lng'] = city['geometry']['bounds']['southwest']['lng']
...
#getBoundingBoxData('dc')
allCities = readCitiesFile('cities')
bigList = []
for city in allCities[:70]:
	data = getBoundingBoxData(city)
	print city
	bigList.extend(data)
for city in allCities[71:140]:
	data = getBoundingBoxData(city)
	print city
	bigList.extend(data)
for city in allCities[140:]:
	data = getBoundingBoxData(city)
	print city
	bigList.extend(data)


for cityData in bigList:
	urlbase = """http://extract.bbbike.org/?lang=en&as=1&submit=extract&format=osm.bz2"""
	cityName = cityData['cityName'].split(',')[0].lower()
	urlbase = urlbase + "&city=" + cityName
	urlbase = urlbase + "&sw_lng=" + str(cityData['sw_lng'])
	urlbase = urlbase + "&sw_lat=" + str(cityData['sw_lat'])
	urlbase = urlbase + "&ne_lng=" + str(cityData['ne_lng'])
	urlbase = urlbase + "&ne_lat=" + str(cityData['ne_lat'])
	urlbase = urlbase + "&email=" + urllib2.quote('scailabtest@gmail.com')
	print urlbase
	response = urllib2.urlopen(urlbase)
	print response.read()

#exportListOfDictionariesAsCSV(bigList, "bcsv1")
